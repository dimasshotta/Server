...
uint64 pc_calc_skilltree_normalize_job_sub( struct map_session_data *sd ){
	...
	if( sd->class_ & MAPID_SUMMONER ){
		// Summoner's skill points for basic skill.
...
static void pc_calcexp(struct map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src)
{
	...
	if (sd->sc.data[SC_EXPBOOST2]) {
		bonus += sd->sc.data[SC_EXPBOOST2]->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.data[SC_EXPBOOST2]->val1 / battle_config.vip_bm_increase);
	}

	if (sd->sc.data[SC_EXPBOOST3]) {
		bonus += sd->sc.data[SC_EXPBOOST3]->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.data[SC_EXPBOOST3]->val1 / battle_config.vip_bm_increase);
	}

	if (sd->sc.data[SC_EXPBOOST4]) {
		bonus += sd->sc.data[SC_EXPBOOST4]->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.data[SC_EXPBOOST4]->val1 / battle_config.vip_bm_increase);
	}
	...
	if (sd->sc.data[SC_JEXPBOOST2])
		bonus += sd->sc.data[SC_JEXPBOOST2]->val1;

	if (sd->sc.data[SC_JEXPBOOST3])
		bonus += sd->sc.data[SC_JEXPBOOST3]->val1;
