...
int npc_isnear_sub(struct block_list* bl, va_list args) {
	...
	if( nd->dynamicnpc.owner_char_id != 0 ) // Rathena Ver
		return 0;
...
int npc_click(struct map_session_data* sd, struct npc_data* nd)
{
	...
	if( npc_is_hidden_dynamicnpc( *nd, *sd ) ) //Rathena Ver
		return 1;
...
bool npc_scriptcont(struct map_session_data* sd, int id, bool closing){
	...
	if( nd != nullptr && nd->dynamicnpc.owner_char_id != 0 ) //Rathena
		nd->dynamicnpc.last_interaction = gettick();
...
struct npc_data* npc_calldynamic( struct npc_data& nd, struct map_session_data& sd ){
	
	int16 new_x, new_y;

	if( !map_search_freecell( &sd.bl, sd.bl.m, &new_x, &new_y, battle_config.feature_dynamicnpc_rangex, battle_config.feature_dynamicnpc_rangey, 0 ) ){
		ShowError( "npc_calldynamic: Unable to find a free cell to duplicate NPC \"%s\" for player \"%s\"(AID: %u, CID: %u)\n", nd.exname, sd.status.name, sd.status.account_id, sd.status.char_id );
		return nullptr;
	}
	...
	CREATE(dnd, struct npc_data, 1);
	
	dnd->bl.prev = dnd->bl.next = NULL;
	dnd->bl.id = npc_get_new_npc_id();
	dnd->bl.m = sd.bl.m;
	dnd->bl.x = new_x;
	dnd->bl.y = new_y;
	dnd->class_ = nd.class_;
	dnd->sc_display = NULL;
	dnd->sc_display_count = 0;
	dnd->dynamicnpc.owner_char_id = sd.status.char_id;
	dnd->dynamicnpc.last_interaction = gettick();
	dnd->dynamicnpc.removal_tid = add_timer( dnd->dynamicnpc.last_interaction + battle_config.feature_dynamicnpc_timeout, npc_dynamicnpc_removal_timer, dnd->bl.id, NULL );
	...
	dnd->speed = 200;
	dnd->bl.type = BL_NPC;
	dnd->subtype = NPCTYPE_SCRIPT;
	dnd->src_id = nd.src_id ? nd.src_id : nd.bl.id;

	dnd->u.scr.xs = nd.u.scr.xs;
	dnd->u.scr.ys = nd.u.scr.ys;
	dnd->u.scr.script = nd.u.scr.script;

	dnd->u.scr.label_list = nd.u.scr.label_list;
	dnd->u.scr.label_list_num = nd.u.scr.label_list_num;

	//Add the npc to its location
	map_addnpc(sd.bl.m, dnd);
	status_change_init(&dnd->bl);
	unit_dataset(&dnd->bl);
	npc_setcells(dnd);
	...
	status_set_viewdata(&dnd->bl, dnd->class_);

	if (map[dnd->bl.m].users)
		clif_spawn(&dnd->bl);
	
	dnd->u.scr.timerid = INVALID_TIMER;
	
	return dnd;
}

int npc_dynamicinit(struct npc_data* nd)
{
	strdb_put(npcname_db, nd->exname, nd);
	...
	char evname[EVENT_NAME_LENGTH];
	safesnprintf(evname, EVENT_NAME_LENGTH, "%s::%s", nd->exname, script_config.init_event_name);
	if ((struct event_data*)strdb_get(ev_db, evname)) {
		npc_event_do(evname);
	}
