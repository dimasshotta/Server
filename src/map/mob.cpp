...
void mob_damage(struct mob_data *md, struct block_list *src, int damage, uint16 skill_id)
{
	...
	if (src && damage > 0) { //Store total damage...
		mobskill_use(md, gettick(), (skill_id<<16), damage);
...
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if (sd->sc.data[SC_ITEMBOOST2])
				drop_rate_bonus += sd->sc.data[SC_ITEMBOOST2]->val1;

			if (sd->sc.data[SC_ITEMBOOST3])
				drop_rate_bonus += sd->sc.data[SC_ITEMBOOST3]->val1;
