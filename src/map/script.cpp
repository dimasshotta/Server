...
/**
 * Create a dynamic NPC for a player.
 * duplicate_dynamic("<NPC name>","<Display name>","<Hidden Name>"{, sprite, <character ID>});
 */
BUILDIN_FUNC(calldynamic)
{
	const char* old_npcname = script_getstr( st, 2 );
	const char* dispname = script_getstr(st, 3);
	const char* hidname = script_getstr(st, 4);
	struct npc_data* nd = npc_name2id( old_npcname );
	
	if( nd == nullptr ){
		ShowError( "buildin_calldynamic: No such NPC '%s'.\n", old_npcname );
		script_pushstrcopy( st, "" );
		return SCRIPT_CMD_FAILURE;
	}
	...
	if (script_hasdata(st, 5)) {
		class_ = script_getnum(st, 5);
	}
	else {
		class_ = nd->class_;
	}
	
	if (class_ == JT_FAKENPC) {
		ShowError("buildin_calldynamic: trying to create a dynamic npc using a FAKE_NPC.\n");
		script_pushstrcopy( st, "" );
		return SCRIPT_CMD_FAILURE;
	}
	...
	strcat(newname, dispname);
	strncat(newname, "#", 1);
	strncat(newname, hidname, strlen(hidname));
	
	if (npc_name2id(newname) != nullptr) {
	...
	struct npc_data* dnd = npc_calldynamic( *nd, *sd );

	safestrncpy(dnd->name, newname, sizeof(dnd->name));
	safestrncpy(dnd->exname, newname, sizeof(dnd->exname));
...
BUILDIN_FUNC(get_reputation_points){
	...
	if( reputation == nullptr ){
		ShowError( "buildin_get_reputation_points: Unknown reputation type %" PRIi64 ".\n", type );
		return SCRIPT_CMD_FAILURE;
	}
