...
int storage_storageopen(struct map_session_data *sd)
{
	...
	if (sd->sc.data[SC_STORECOUNT])
		sd->storage.max_amount = sd->sc.data[SC_STORECOUNT]->val1 + MIN_STORAGE;
...
void storage_premiumStorage_open(struct map_session_data *sd) {
	...
	if (sd->sc.data[SC_STORECOUNT])
		sd->premiumStorage.max_amount = sd->sc.data[SC_STORECOUNT]->val1 + MIN_STORAGE;
